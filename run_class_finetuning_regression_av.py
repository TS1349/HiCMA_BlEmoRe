...
from engine_for_finetuning_regression_av import train_one_epoch, validation_one_epoch, final_test, merge
...
def get_args():
    parser = argparse.ArgumentParser('HiCMAE fine-tuning and evaluation script for audio-visual emotion recognition', add_help=False)
    ...
    parser.add_argument('--attn_type', default='joint', choices=['joint'],
    ...
    ## for frame-level prediction task
    parser.add_argument('--keep_temporal_dim', action='store_true', default=False)
    ## add head activation function support for regression task
    parser.add_argument('--head_activation_func', type=str, default=None, choices=['sigmoid', 'relu', 'tanh'])
    ...
    parser.add_argument('--fusion_depth', required=True, type=int,
...
def main(args, ds_init):
    ...
    if args.finetune:
        ...
        if 'encoder_audio.pos_embed' in checkpoint_model:
            ...
            if orig_temporal_size != new_temporal_size: # assert the freq dim is fixed (i.e., 128//16=8)
                ...
                # pos_tokens = torch.nn.functional.interpolate(
                #     pos_tokens, size=(new_size, freq_size), mode='bicubic', align_corners=False)
    ...
    if args.layer_decay < 1.0:
        assigner = LayerDecayValueAssignerForAudioVisual(list(args.layer_decay ** (num_layers + 1 - i) for i in range(num_layers + 2)),
                                                         model_without_ddp.get_num_modality_specific_layers())
    else:
        assigner = None
    ...
    if args.eval:
        ...
        if global_rank == 0:
            ...
            metrics_dict, pred_dict = merge(args.output_dir, num_tasks, args)
            print(f"Final performance of the network on the {len(dataset_test)} test videos:\n"
                  f"ACC: {metrics_dict['acc'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['acc'][1]])}), \n"
                  f"MAE: {metrics_dict['mae'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['mae'][1]])}), \n"
                  f"MSE: {metrics_dict['mse'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['mse'][1]])}), \n"
                  f"RMSE: {metrics_dict['rmse'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['rmse'][1]])}), \n"
                  f"PCC: {metrics_dict['pcc'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['pcc'][1]])}), \n"
                  f"CCC: {metrics_dict['ccc'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['ccc'][1]])})")
            log_stats = {
                'Final ACC': f"{metrics_dict['acc'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['acc'][1]])}), \n",
                'Final MAE': f"{metrics_dict['mae'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['mae'][1]])}), \n",
                'Final MSE': f"{metrics_dict['mse'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['mse'][1]])}), \n",
                'Final RMSE': f"{metrics_dict['rmse'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['rmse'][1]])}), \n",
                'Final PCC': f"{metrics_dict['pcc'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['pcc'][1]])}), \n",
                'Final CCC': f"{metrics_dict['ccc'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['ccc'][1]])})", }
            # more metrics
    ...
    best_metric = -1e8 if args.val_metric in ['acc', 'pcc', 'ccc'] else 1e8
    best_epoch = None
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if data_loader_val is not None:
            ...
            print(f"Accuracy of the network on the {len(dataset_val)} val videos: {test_stats['acc']:.4f}")
            if (args.val_metric in ['acc', 'pcc', 'ccc'] and best_metric < test_stats[f"{args.val_metric}_total"]) or \
            (args.val_metric in ['mae', 'rmse'] and best_metric > test_stats[f"{args.val_metric}_total"]):
                best_metric = test_stats[f"{args.val_metric}_total"] # use total value, not batch average
                best_epoch = epoch
                if args.output_dir and args.save_ckpt:
                    utils.save_model(
                        args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
            ...
            print(f"==> Best '{args.val_metric.lower()}': {best_metric:.4f} (epoch={best_epoch})")
            if log_writer is not None:
                log_writer.update(val_acc=test_stats['acc'], head="perf", step=epoch)
                log_writer.update(val_acc_total=test_stats['acc_total'], head="perf", step=epoch)
                log_writer.update(val_mae=test_stats['mae'], head="perf", step=epoch)
                log_writer.update(val_mae_total=test_stats['mae_total'], head="perf", step=epoch)
                log_writer.update(val_mse=test_stats['mse'], head="perf", step=epoch)
                log_writer.update(val_mse_total=test_stats['mse_total'], head="perf", step=epoch)
                log_writer.update(val_rmse=test_stats['rmse'], head="perf", step=epoch)
                log_writer.update(val_rmse_total=test_stats['rmse_total'], head="perf", step=epoch)
                log_writer.update(val_pcc=test_stats['pcc'], head="perf", step=epoch)
                log_writer.update(val_pcc_total=test_stats['pcc_total'], head="perf", step=epoch)
                log_writer.update(val_ccc=test_stats['ccc'], head="perf", step=epoch)
                log_writer.update(val_ccc_total=test_stats['ccc_total'], head="perf", step=epoch)
    ...
    # load best model and perform test again
    preds_file = os.path.join(args.output_dir, str(global_rank) + '_best.txt')
    best_ckpt_path = Path(args.output_dir) / ('checkpoint-%s.pth' % 'best')
    best_ckpt = torch.load(best_ckpt_path, map_location='cpu')
    model_without_ddp.load_state_dict(best_ckpt['model'])
    print("Resume checkpoint %s" % best_ckpt_path)
    test_stats = final_test(data_loader_test, model, device, preds_file, args)
    ...
    if global_rank == 0:
        ...
        # me: original merge
        metrics_dict, pred_dict = merge(args.output_dir, num_tasks, args)
        print(f"Final performance of the network on the {len(dataset_test)} test videos using last epoch model:\n"
              f"ACC: {metrics_dict['acc'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['acc'][1]])}), \n"
              f"MAE: {metrics_dict['mae'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['mae'][1]])}), \n"
              f"MSE: {metrics_dict['mse'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['mse'][1]])}), \n"
              f"RMSE: {metrics_dict['rmse'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['rmse'][1]])}), \n"
              ...
              f"CCC: {metrics_dict['ccc'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['ccc'][1]])})")
        log_stats = {
            'Final ACC (last epoch)': f"{metrics_dict['acc'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['acc'][1]])}), \n",
            'Final MAE (last epoch)': f"{metrics_dict['mae'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['mae'][1]])}), \n",
            'Final MSE (last epoch)': f"{metrics_dict['mse'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['mse'][1]])}), \n",
            'Final RMSE (last epoch)': f"{metrics_dict['rmse'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['rmse'][1]])}), \n",
            'Final PCC (last epoch)': f"{metrics_dict['pcc'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['pcc'][1]])}), \n",
            'Final CCC (last epoch)': f"{metrics_dict['ccc'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['ccc'][1]])})",
        }
        # me: merge the results of best model
        metrics_dict, pred_dict_best = merge(args.output_dir, num_tasks, args, best=True)
        print(f"Final performance of the network on the {len(dataset_test)} test videos using best epoch model:\n"
              f"ACC: {metrics_dict['acc'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['acc'][1]])}), \n"
              f"MAE: {metrics_dict['mae'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['mae'][1]])}), \n"
              f"MSE: {metrics_dict['mse'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['mse'][1]])}), \n"
              f"RMSE: {metrics_dict['rmse'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['rmse'][1]])}), \n"
              ...
              f"CCC: {metrics_dict['ccc'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['ccc'][1]])})")
        log_stats_best = {
            'Final ACC (best epoch)': f"{metrics_dict['acc'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['acc'][1]])}), \n",
            'Final MAE (best epoch)': f"{metrics_dict['mae'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['mae'][1]])}), \n",
            'Final MSE (best epoch)': f"{metrics_dict['mse'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['mse'][1]])}), \n",
            'Final RMSE (best epoch)': f"{metrics_dict['rmse'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['rmse'][1]])}), \n",
            'Final PCC (best epoch)': f"{metrics_dict['pcc'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['pcc'][1]])}), \n",
            'Final CCC (best epoch)': f"{metrics_dict['ccc'][0]:.4f} ({', '.join([f'{e:.4f}' for e in metrics_dict['ccc'][1]])})",
        }
        #  more metrics
        if args.output_dir and utils.is_main_process():
            with open(os.path.join(args.output_dir, "log.txt"), mode="a", encoding="utf-8") as f:
