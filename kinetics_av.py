...
class VideoMAE(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            next_idx = random.randint(0, self.__len__() - 1)
            print(
                f"==> Exception '{e}' occurred when processed '{directory}', move to random next one (idx={next_idx}).")
            return self.__getitem__(next_idx)
        # get audio mask
        mask_audio = self.mask_generator_audio()

        # process_data: (C,T,H,W)
        # mask: (N,)
        # audio_data: (C,T,F)
        return (process_data, mask, audio_data, mask_audio)
